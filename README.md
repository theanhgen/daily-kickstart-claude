...
- **For AI agents** — [site/webmcp.js](site/webmcp.js) registers eight read-only [WebMCP](https://developer.chrome.com/docs/ai/webmcp) tools where Chrome exposes the API (the WebMCP origin trial, or `chrome://flags/#enable-webmcp-testing`), Seven read the archive: `get_latest_haikus`, `search_haikus` (text), `get_haikus_for_word` (whole words), `search_by_word` (the vocabulary), `get_word_cloud`, `get_word_trends` (uses per month or week) and `compare_engines` (the Archive page's per-engine stats for any date range, from copies of main.js's word and mood code that a test holds to the same numbers). `get_free_model_bench` reads the bench and is marked untrusted content. [site/llms.txt](site/llms.txt) summarises the site and its data files. The site has no forms, so there is nothing to annotate declaratively.
